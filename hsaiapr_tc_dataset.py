...
TRAIN_OUTPUT_DATASET_PATH = 'ont_train_saiapr.csv'
TEST_OUTPUT_DATASET_PATH = 'ont_test_saiapr.csv'
OUTPUT_TAGS_PATH = 'ont_iapr_tags.txt'
TEST_DATA_NUM = 1000
...
def create_kv_dataset():
    ...
    for file_path in files:
        if file_path != '.DS_Store':
            with open(DATASET_DEIR_PATH + '/saiapr_tc-12/' + file_path + '/ontology_path.txt', 'r') as f:
                for tag in f.read().splitlines():
                    elements = tag.split()
                    if len(elements) > 0:
                        ontology_txt = elements[2]
                        ontology = ontology_txt[10:].split('->')
                        words = [word for word in ontology if word != 'entity']
                        cleaned_words = list(map(lambda word: word[1:] if word.startswith('_') else word, words))
                        for word in cleaned_words:
                            if not word in data_list[int(elements[0])]:
                                data_list[int(elements[0])].append(word)
